...
class SegRetino(object):
    ...
    def inference(self, set_weight_dir = 'unet.pth', path = 'output.png', blend = True, blend_path = 'blend.png'):
        ...
        # Save clean segmentation without grey bar
        clean_segmentation = pred_y * 255
        cv2.imwrite(path, clean_segmentation)

        image = cv2.resize(image, self.size)

        if blend:
            # pred_y is already RGB after mask_parse, so resize and blend directly
            segmentation_rgb = cv2.resize(clean_segmentation.astype(np.uint8), self.size)
            blend = cv2.addWeighted(image, 0.8, segmentation_rgb, 0.5, 0)
